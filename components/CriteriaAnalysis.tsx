                ...
                {absence.map((row, index) => (
                  <tr
                    ...
                    className={`border-b border-zinc-700/50 ${
                      index === 0 ? 'bg-red-950/30' : 'hover:bg-zinc-700/30'
                    } transition`}
                  >
                    <td
                      className={`py-2.5 pr-3 font-medium ${
                        index === 0 ? 'text-red-400' : 'text-zinc-300'
                      }`}
                    >
                      {row.label}
                    </td>
                    <td className="py-2.5 px-2 text-center text-zinc-400">{row.weight}</td>
                    <td className="py-2.5 px-2 text-center text-zinc-300">{row.absentCount}</td>
                    ...
                    <td
                      className={`py-2.5 pl-2 text-right font-semibold ${
                        index === 0 ? 'text-red-400' : 'text-zinc-200'
                      }`}
                    >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {impact.map((row) => (
                  ...
                  >
                    <td className="py-2.5 pr-3 text-zinc-300 font-medium">{row.label}</td>
                    <td className="py-2.5 px-2 text-center text-zinc-300">
